...
from app.converter import clean_lines
from httpx import ASGITransport, AsyncClient


class TestCleanLines:
    """Tests for the clean_lines function."""
    ...
    def test_detects_numbered_heading(self):
        result = clean_lines(["1. Introduction"])
    ...
    def test_all_caps_with_punctuation_not_heading(self):
        ...
        with patch.object(settings, "markdown_heading_detection", True):
            result = clean_lines(["ATTENTION: LIRE AVANT D'UTILISER"])
    ...
    def test_all_caps_with_digits_not_heading(self):
        ...
        with patch.object(settings, "markdown_heading_detection", True):
            result = clean_lines(["VERSION 2.0"])
    ...
    def test_preserves_bullet_list(self):
        result = clean_lines(["* item 1", "- item 2", "• item 3"])
        assert result == ["* item 1", "- item 2", "• item 3"]

    def test_preserves_normal_text(self):
        result = clean_lines(["Ceci est un paragraphe normal."])
