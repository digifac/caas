...
async def _convert_pdf_stream(file_bytes: bytes) -> AsyncGenerator[str, None]:
...
async def _convert_docx_stream(file_bytes: bytes) -> AsyncGenerator[str, None]:
...
async def _convert_odt_stream(file_bytes: bytes) -> AsyncGenerator[str, None]:
...
async def _convert_html_stream(file_bytes: bytes) -> AsyncGenerator[str, None]:
...
async def _convert_xlsx_stream(file_bytes: bytes) -> AsyncGenerator[str, None]:
...
async def _convert_pptx_stream(file_bytes: bytes) -> AsyncGenerator[str, None]:
    """Stream PPTX → Markdown conversion.

    The PPTX converter processes the entire presentation at once, so we split
    the result into chunks of approximately `streaming_chunk_size` bytes.
    """
...
async def _convert_ods_stream(file_bytes: bytes) -> AsyncGenerator[str, None]:
...
async def _convert_odp_stream(file_bytes: bytes) -> AsyncGenerator[str, None]:
...
async def convert_stream(file_bytes: bytes, ext: str) -> AsyncGenerator[str, None]:
    ...
    Yields:
        SSE-formatted strings containing Markdown chunks.
    ...
    start_event = json.dumps(
        {"format": ext, "size_bytes": len(file_bytes), "status": "started"}
    )
